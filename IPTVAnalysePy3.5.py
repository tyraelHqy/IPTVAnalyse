...
def getM3uSorted():
    ...
    # print("******************************channelNameStr*************************************")
    # print(channelNameSorts)
    # print("******************************channelUrlSorts***********************************")
    # print(channelUrlSorts)
    dictChannelSort = dict(zip(channelNameSorts, channelUrlSorts))
    # print("*****************************dictChannelSort******************************")
    # print(dictChannelSort)
    txt.close()
    return dictChannelSort, channelNameSorts
...
def updateDictChannelSortList(dictChannel, dictChannelSort, channelNameSorts):
    updateList = []
    insertList = []
    ...
    m3uFileOut(dictChannelSort, False, channelNameSorts)
    return
...
def m3uFileOut(dict, needSorted, channelNameSorts):
    ...
    if(needSorted):
        for key, value in sorted(dict.items()):
            if len(value)<50:
                ...
                print(key)
                f.write("http://" + ipAndPort + "/"+ protocol +"/" + value + '\n')
    else:
        for channelNameSort in (channelNameSorts):
            if len(dict[channelNameSort]) < 50:
                f.write("#EXTINF:-1," + channelNameSort + '\n')
                print(channelNameSort)
                f.write("http://" + ipAndPort +  "/"+ protocol +"/"  + dict[channelNameSort] + '\n')
    f.close()
...
def IPTVAnalyse():
    ...
    try:
        dictChannelSort , channelNameSorts = getM3uSorted()
    except FileNotFoundError:
        m3uFileOut(dictChannel,True,(range(1,1000)))
        return
    updateDictChannelSortList(dictChannel, dictChannelSort,channelNameSorts)
    pass
